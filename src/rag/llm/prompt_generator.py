...
class ChatTemplatePromptGenerator:
    """Generates structured chat templates with retrieved contexts for the LLM."""
    ...
    def generate_prompt(
        ...
        Returns:
            A list of chat messages (list of role/content dicts) for each query.
        """
    ...
    def _build_messages(
        ...
        """Constructs system and user message pairs for each query search result.
    ...
    def _generate_user_prompt(
        ...
        """Generates the user message dictionary containing query and formatted context.
    ...
    def _system_prompt(self) -> dict[str, str]:
        """Gets the system instructions prompt dictionary for the assistant's behavior.

        Returns:
            A dictionary with 'role': 'system' and 'content' instruction string.
        """
        return {
            "role": "system",
            "content": (
